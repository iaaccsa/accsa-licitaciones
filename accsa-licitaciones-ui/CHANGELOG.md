...
- Si al arrancar un paso del análisis se corta la conexión con la infraestructura que ejecuta los trabajos, ahora se reintenta en vez de dar el análisis por fallido en el primer intento. Un corte de red de un segundo mataba el análisis entero y había que reintentarlo a mano. El resto de las llamadas ya se reintentaban; faltaba justamente la que lanza cada paso.
- El aviso de un análisis que terminó sin nada para evaluar vuelve a mostrarse. El motivo del corte se guardaba bien, pero la consulta que alimenta al listado y al detalle no lo devolvía, así que esos análisis se veían como "Completado" en verde y sin explicación. Los que ya están cerrados con ese motivo también quedan corregidos, sin volver a procesarlos.
