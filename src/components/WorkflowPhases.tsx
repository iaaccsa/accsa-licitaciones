    ...
    return (
        <div className="flex flex-col items-center text-center w-[140px] sm:w-[170px] shrink-0">
            <div className="relative w-14 h-14 flex items-center justify-center">
                ...
                <div className={`relative w-10 h-10 rounded-full flex items-center justify-center font-semibold text-sm transition-colors ${nodeStyle}`}>
                    {uiStatus === "done" ? (
            ...
            <div className={`mt-1.5 text-[10px] uppercase tracking-wider font-semibold ${statusColor}`}>
...
export default function WorkflowPhases({ analysisId }: WorkflowPhasesProps) {
    ...
    const sorted = useMemo(
        () => [...phases].filter((p) => p.type === "processing").sort((a, b) => a.order - b.order),
        [phases]
    );

    const approvalPhases = useMemo(
        () => [...phases].filter((p) => p.type === "approval").sort((a, b) => a.order - b.order),
        [phases]
    );
            ...
            <div className="flex items-start">
                {sorted.map((stage, idx) => {
                    ...
                    const approvalBetween = showConnector
                        ? approvalPhases.find((a) => a.order > prev.order && a.order < stage.order)
                        : undefined;

                    return (
                        <FragmentKey key={stage.code}>
                            {showConnector && (
                                <Connector prev={prev} current={stage} approvalPhase={approvalBetween} />
                            )}
