...
class AdmissibilityResultRepository(BaseRepository):
    def __init__(self):
        super().__init__("admissibility_results")

    def validate_requirement_ids(self, analysis_id: UUID, req_ids: Set[str]) -> Set[str]:
        """Returns the subset of req_ids that do NOT exist in admissibility_requirements for analysis_id."""
        response = (
            supabase.table("admissibility_requirements")
            .select("id")
    ...
    def get_by_proposal(self, proposal_id: UUID) -> List[Dict[str, Any]]:
        response = (
            supabase.table(self.table_name)
            .select("*, admissibility_requirements(*)")
            .eq("proposal_id", str(proposal_id))
            .execute()
        )
...
admissibility_result_repository = AdmissibilityResultRepository()
