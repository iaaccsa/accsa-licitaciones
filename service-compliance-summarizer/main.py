...
# LLM calls
# ---------------------------------------------------------------------------
def _call_gemini_summary(gemini: genai.Client, user_prompt: str) -> str:
...
def _call_openai_summary(openai_client: OpenAI, user_prompt: str) -> str:
...
def call_llm_summary(
    ...
    for attempt in range(LLM_RETRY_ATTEMPTS + 1):
        try:
            text = _call_gemini_summary(gemini, user_prompt)
            logger.info(f"Gemini summary generated on attempt {attempt}.")
            return text
        except Exception as e_gemini:
            logger.warning(f"Attempt {attempt}: Gemini failed ({e_gemini}), trying OpenAI...")
            try:
                text = _call_openai_summary(openai_client, user_prompt)
                logger.info(f"OpenAI fallback summary generated on attempt {attempt}.")
                return text
